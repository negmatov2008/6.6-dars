...
function Jupeter() {
  const [planetImg, setPlanetImg] = useState(Api.data[4].images.planet);
  const [paragraph, setParagraph] = useState(Api.data[4].overview);
  const [index, setIndex] = useState(0);
  function handleClick(i) {
    setIndex(i);
    if (i == 0) {
      setPlanetImg(Api.data[4].images.planet);
      setParagraph(Api.data[4].overview);
    } else if (i == 1) {
      setPlanetImg(Api.data[4].images.internal);
      setParagraph(Api.data[4].structure);
    } else if (i == 2) {
      setPlanetImg(Api.data[4].images.geology);
      setParagraph(Api.data[4].geology);
    }
  }
    ...
    <div className="container">
      <div className="main">
        ...
        <div className="main-content">
          <h1>{Api.data[4].name}</h1>
          <p>{paragraph.content}</p>
          <span>
            Source :{" "}
            <a href={paragraph.source} target="_blank">
              Wikipedia
            </a>
          </span>
          <div className="sections">
            {sections.map((section, i) => (
              <div
                ...
                className={index == i ? "section section-active" : "section"}
              >
      ...
      <div className="footer">
        <div className="box">
          ...
          <span>{Api.data[4].rotation}</span>
        </div>
        <div className="box">
          <span>REVOLUTION TIME</span>
          <span>{Api.data[4].revolution}</span>
        </div>
        <div className="box">
          <span>radius</span>
          <span>{Api.data[4].radius}</span>
        </div>
        <div className="box">
          <span>AVERAGE TEMP.</span>
          <span>{Api.data[4].temperature}</span>
        </div>
      </div>
    </div>
  );
